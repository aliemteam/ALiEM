...
class PostHeader extends React.Component<Props, State> {
    ...
    constructor() {
        ...
        const postContent: HTMLDivElement | null = document.querySelector('.post-content');
        if (!postContent) {
            throw new Error('Post content div element not found.');
        }
        this.postContentDiv = postContent;
        (document as any).addEventListener('scroll', this.handleProgress, { passive: true });
    }
    ...
    handleProgress = (_e: UIEvent) => {
        ...
        this.setState(prevState => ({ ...prevState, isActive, progressCurrent, progressMax }));
    };

    render() {
        return (
            <header
                className={`header__posts ${this.state.isActive ? 'header__posts--active' : ''}`}
            >
                <div className="header__posts__container">
                    ...
                    <div className="header__posts__share-link-container">
                        <div className="header__posts__share-label">Share this</div>
                ...
                <progress value={this.state.progressCurrent} max={this.state.progressMax} />
            </header>
        );
    }
}
